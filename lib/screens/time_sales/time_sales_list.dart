...
import 'package:mint_bird_app/screens/dashboard/API/delete_timesale.dart';
...
import 'package:mint_bird_app/screens/time_sales/API/get_timesales_details.dart';
import 'package:mint_bird_app/screens/time_sales/models/time_sales_model.dart';
import 'package:mint_bird_app/screens/time_sales/time_sale_detail.dart';
import 'package:mint_bird_app/utils/app_strings.dart';
import 'package:mint_bird_app/utils/m_colors.dart';
import 'package:mint_bird_app/widgets/delete_popup.dart';

class TimeSalesList extends StatefulWidget {
  final ScrollController scrollController;

  TimeSalesList(this.scrollController);

  @override
  _TimeSalesListState createState() => _TimeSalesListState();
}

class _TimeSalesListState extends State<TimeSalesList> {
          ...
          : ListView.builder(
              ...
              itemBuilder: (context, index) {
                return Column(
                  children: [
                    dimeSalesCard(
                        dashboardController.listOfTimesell[index], index),
                    dashboardController.bottomLoader.value &&
                            index + 1 ==
                                dashboardController.listOfTimesell.length
                        ? productShimmer(verticalPadding: 0)
                        : SizedBox()
                  ],
                );
              },
              itemCount: dashboardController.listOfTimesell.length);
    });
  }

  Widget dimeSalesCard(TimesellUpsell data, index) {
    return InkWell(
      onTap: () {
        getUserTimeSaleDetailService(data.id);
        Get.to(() => TimeSaleDetails(
              title: data.timesaleName,
            ));
      },
      child: Container(
