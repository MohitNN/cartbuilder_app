...
import 'package:mint_bird_app/screens/product/API/scarcity/scarcity_dime__sale_product.dart';
import 'package:mint_bird_app/utils/api_utils.dart';
import 'package:mint_bird_app/widgets/get_toast.dart';

Future<bool> updateScarCityDimeSalesDetail({String productBody}) async {
  DimeSellController dimeSellController = Get.put(DimeSellController());
  dimeSellController.detailLoader.value = true;
  var response = await http.post(
      Uri.parse(APIUtils.baseUrl + APIUtils.updateScarcityDime),
      headers: {
        "Authorization": "Bearer ${authController.userToken.value}",
      },
      body: {
        "id": dimeSellController.dimeSaleId.value,
  ...
  if (response.statusCode == 200) {
    if (decodedData["status"] == "Success") {
      updateScarDimeSalesProducts(productBody);
      return true;
    } else {
